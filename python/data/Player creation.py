...
df_players = pd.read_csv(
    "Data/atp_players.csv", header=None, names=[1, 2, 3, 4, 5, 6], encoding="ISO-8859-1"
)
    ...
    for row in range(len(df_matches_year)):
        ...
        try:
            ...
            if (
                ...
                and w_bp_Saved == w_bp_Saved
                and w_SvGms == w_SvGms
            ):
                winner.update_breakpoint_faced_and_savec(
                    w_bp_Faced, w_bp_Saved, w_SvGms
                )
        except:
...
for player in players_list_dict.values():
    ...
    print(
        ...
        player.last_matches,
        " *** Victory Percentage : ",
        player.victory_percentage,
        "%",
    )
    print("           *** Ace Percentage : ", player.ace_percentage, "%")
    print(
        "           *** Double Fault Percentage : ", player.doublefault_percentage, "%"
    )
    print(
        "           *** Serve Point Won Percentage : ",
        player.overall_win_on_serve_percentage,
        "%",
    )
    print(
        "           *** Serve Point In Percentage : ",
        player.first_serve_success_percentage,
        "%",
    )
    print(
        "           *** BreakPoint Faced : ",
        player.breakpoint_faced_percentage,
        "%",
        "   *** Breakpoint Saved : ",
...
df_data_players = pd.DataFrame(
    columns=[
        ...
        "height",
        "victory_percentage",
        "clay_victory_percentage",
        "carpet_victory_percentage",
        "grass_victory_percentage",
        "hard_victory_percentage",
        "ace_percentage",
        "doublefault_percentage",
...
for player in players_list_dict.keys():
    df_to_add = pd.DataFrame(
        [players_list_dict[player].get_data_profile()],
        columns=[
            "name",
            "id",
            "ranking",
